"""
Replay buffers pour le World Model (observations IMAGE).

Trois variantes, toutes en stockage uint8 (4x moins que float32) :

  ImageReplayBuffer     : buffer plat, host (numpy). Utilise par les micro-benchs.
  ImageReplayBufferJAX  : per-env, resident sur le device. Zero transfert host<->device
                          dans la hot loop, mais demande ~12.3 GB de VRAM a 1M.
  ImageReplayBufferCPU  : per-env, resident en RAM host. Variante pour les devices a
                          faible VRAM et pour le TPU (la prod utilise celle-ci via
                          --buffer_device cpu).

Les deux variantes per-env stockent par environnement, (n_envs, per_env_cap, ...) : une
sequence samplee vit donc entierement dans UN env = vraie trajectoire temporelle. Un
stockage plat entrelacerait les envs et produirait des trajectoires fictives (bug reel,
corrige a l'audit v16 : la recon convergeait quand meme, ce qui masquait le probleme).

Le flag `dones` marque les frontieres d'episode ; les composants downstream (RSSM,
imagination) s'en servent pour reset l'etat.
"""
